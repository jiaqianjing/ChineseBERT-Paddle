...
train_ds = load_dataset(read, data_path=f'{data_prefix}/ChineseBERT-Paddle/data/CMRC/raw_data/train.json', lazy=False)
dev_ds = load_dataset(read, data_path=f'{data_prefix}/ChineseBERT-Paddle/data/CMRC/raw_data/dev.json', lazy=False)
test_ds = load_dataset(read, data_path=f'{data_prefix}/ChineseBERT-Paddle/data/CMRC/raw_data/test.json', lazy=False)
print(f"train_ds len: {len(train_ds)}, dev_ds len: {len(dev_ds)}, test_ds len: {len(test_ds)}")
...
def evaluate(model, data_loader, args, epoch, dataset_name="dev"):
    ...
    with open(f'{output_dir}/{dataset_name}_prediction_{epoch}.json', "w", encoding='utf-8') as writer:
        writer.write(
            json.dumps(
