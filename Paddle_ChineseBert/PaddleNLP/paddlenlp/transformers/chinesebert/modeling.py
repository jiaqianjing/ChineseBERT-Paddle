...
def test_pinyin_embeddings():
    ...
    sentence = '我喜欢猫'
    CHINESEBERT_PATH = '../pretrain_models/paddle/ChineseBERT-base/'
    ...
    print(
        f"input_ids: {input_ids.cpu().detach().numpy()}\npinyin_ids: {pinyin_ids.cpu().detach().numpy()}"
    )

    length = input_ids.shape[0]
    print(f"length: {length}")
    print("paddle size(shape)", input_ids.shape)
    print(input_ids)
    pinyin_ids = pinyin_ids.reshape((1, length, 8))
    print(pinyin_ids, pinyin_ids.shape)

    hidden_size = 768
    config_path = '../pretrain_models/paddle/ChineseBERT-base/config/'
    pinyin_embeddings = PinyinEmbedding(embedding_size=128,
                                        pinyin_out_dim=hidden_size,
                                        config_path=config_path)
    pinyin_emb = pinyin_embeddings(pinyin_ids)
    print(">>>>pinyin_emb<<<<")
    print(pinyin_emb, pinyin_emb.shape)


def test_glyph_embedding():
    config_path = '../pretrain_models/paddle/ChineseBERT-base/config/'
    font_files = []
    for file in os.listdir(config_path):
        if file.endswith(".npy"):
    ...
    p_emb_w = glyph_embeddings.state_dict()['embedding.weight'].cpu().detach(
    ).numpy()
    print(f"paddle glyph_embeddings: {glyph_embeddings}")


if __name__ == "__main__":
    # test_pinyin_embeddings()
    # test_glyph_embedding()
    glyce_bert_model = GlyceBertModel(
        name_or_path='../pretrain_models/paddle/ChineseBERT-base')
    paddle_state_dict = paddle.load(
        '../pretrain_models/paddle/ChineseBERT-base/chinese_bert_model.pdparams'
    )
    glyce_bert_model.load_dict(paddle_state_dict)
    print(glyce_bert_model)
